...
void MicroView::writeNoAdvance(uint8_t c) {
	writeNoAdvance(c, foreColor, drawMode);
}

void MicroView::writeNoAdvance(uint8_t c, uint8_t color, uint8_t mode) {
	drawChar(cursorX, cursorY, c, color, mode);
}

// void MicroView::backspace() {
// 	if (cursorX < fontWidth + 1) {
// 		if (cursorY < fontHeight) {
// 			return;
// 		}
// 		cursorX = (fontWidth + 1) * (LCDWIDTH / (fontWidth + 1));
// 		cursorY -= fontHeight;
// 	} else {
// 		cursorX -= fontWidth + 1;
// 	}
